...
def make_last_layers(filters_list, in_filters, out_filter):
    # 使用1*1卷积调整通道数，减少网络的参数量
    # 1*1*少通道---->3*3*多通道--->1*1*少通道---->3*3*多通道---->1*1*少通道---->3*3*多通道----->1*1*最终结果
    m = nn.ModuleList([
...
class YoloBody(nn.Module):
    ...
    def forward(self, x):
        # layer_in最终是这一层得预测，out_branch是用于特征融合的特征图
        ...
        # concate的维度是1，第0维是batch, concate后256+512
        ...
        # concate的维度是1，第0维是batch, concate后128+256
